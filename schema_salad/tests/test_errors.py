...
def test_error_message1() -> None:
    ...
    match = (
        r"""^.+test1\.cwl:2:1: Object\s+'.+test1\.cwl'\s+is\s+not\s+valid\s+"""
        + r"""because\s+tried\s+'Workflow'\s+but
...
def test_error_message3() -> None:
    ...
    match = r"""
^.+test3\.cwl:6:1: checking\s+field\s+'outputs'
.+test3\.cwl:7:3:\s+checking\s+object\s+'.+test3\.cwl#bar'
\s+Field\s+'type'\s+references\s+unknown\s+identifier\s+'xstring',\s+tried\s+file://.+/tests/test_schema/test3\.cwl#xstring$"""[  # noqa: B950
        1:
    ]
    with pytest.raises(ValidationException, match=match):
...
def test_error_message4() -> None:
    ...
    match = r"""
^.+test4\.cwl:6:1: checking\s+field\s+'outputs'
...
.+test5\.cwl:8:9:\s+item\s+is\s+invalid\s+because
...
\s+tried\s+array\s+of\s+<WorkflowStep>\s+but
.+test7\.cwl:9:3:\s+item\s+is\s+invalid\s+because
\s+\*\s+missing\s+required\s+field\s+'run'
.+test7\.cwl:10:5:\s+\*\s+invalid\s+field\s+'scatter_method',\s+expected\s+one\s+"""
...
def test_error_message8() -> None:
    ...
    match = (
        r"""
^.+test8\.cwl:8:1:\s+checking\s+field\s+'steps'
...
\s+tried\s+array\s+of\s+<WorkflowStep>\s+but
.+test10\.cwl:9:3:\s+item\s+is\s+invalid\s+because
\s+\*\s+missing\s+required\s+field\s+'run'
...
def test_error_message11() -> None:
    ...
    match = (
        r"""
^.+test11\.cwl:8:1:\s+checking\s+field\s+'steps'
.+test11\.cwl:9:3:\s+checking\s+object\s+'.+test11\.cwl#step1'
.+test11\.cwl:10:5:\s+"""
        r"""Field\s+'run'\s+contains\s+undefined\s+reference\s+to\s+'file://.+/tests/test_schema/blub\.cwl'$"""[
            1:
        ]
    )
    with pytest.raises(ValidationException, match=match):
...
def test_error_message15() -> None:
    ...
    match = (
        r"""^.+test15\.cwl:3:1:\s+Object\s+'.+test15\.cwl'\s+is\s+not\s+valid\s+because\s+tried\s+'CommandLineTool'\s+but
.+test15\.cwl:6:1:\s+the\s+'inputs'\s+field\s+is\s+not\s+valid\s+because
.+test15\.cwl:7:3:\s+item\s+is\s+invalid\s+because
.+test15\.cwl:9:5:\s+the\s+'inputBinding'\s+field\s+is\s+not\s+valid\s+because\s+tried\s+CommandLineBinding\s+but
