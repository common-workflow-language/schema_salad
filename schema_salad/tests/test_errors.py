...
.+test7\.cwl:9:5:           \* invalid\s+field\s+`scatter_method`,\s+expected one of:\s+'id', 'in', 'out',\s+'requirements',\s+'hints', 'label',\s+'doc',\s+'run',\s+'scatter',\s+'scatterMethod'$'''[1:]
    with pytest.raises(ValidationException, match=match):
        load_and_validate(document_loader, avsc_names,
                          six.text_type(get_data("tests/"+t)), True)
...
@pytest.mark.skip("See https://github.com/common-workflow-language/common-workflow-language/issues/734")
