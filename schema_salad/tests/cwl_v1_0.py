...
class RecordField(Saveable):
    ...
    def __eq__(self, other: Any) -> bool:
        if isinstance(other, RecordField):
            return bool(
                self.name == other.name and self.doc == other.doc and self.type_ == other.type_
            )
        return False
...
class ArraySchema(Saveable):
    ...
    def save(
        ...
        if self.items is not None:
            r["items"] = save(self.items, top=False, base_url=base_url, relative_uris=relative_uris)
        if self.type_ is not None:
            r["type"] = save(self.type_, top=False, base_url=base_url, relative_uris=relative_uris)
...
class InputEnumSchema(EnumSchema, InputSchema):
    ...
    def __hash__(self) -> int:
        return hash((self.symbols, self.type_, self.label, self.name, self.inputBinding))
...
class InputParameter(Parameter):
    ...
    def fromDoc(
        ...
        if _errors__:
            raise ValidationException("tried `InputParameter` but", None, _errors__)
        _constructed = cls(
            label=label,
            secondaryFiles=secondaryFiles,
...
class OutputParameter(Parameter):
    ...
    def fromDoc(
        ...
        if _errors__:
            raise ValidationException("tried `OutputParameter` but", None, _errors__)
        _constructed = cls(
            label=label,
            secondaryFiles=secondaryFiles,
            streamable=streamable,
