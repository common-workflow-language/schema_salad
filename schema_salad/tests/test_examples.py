...
def test_scoped_ref():
    ...
    assert {
        ...
        'steps': [{
            ...
            "out": ["http://example2.com/#step2/out"], }]} == ra
...
def test_yaml_float_test():
    assert ruamel.yaml.safe_load("float-test: 2e-10")["float-test"] == \
        2e-10
...
def test_scoped_id():
    ...
    assert {'id': 'http://example.com/#foo',
            'bar': {'id': 'http://example.com/#foo/baz'},} == ra
    ...
    assert {'location': 'http://example.com/foo',
            'bar': {'location': 'http://example.com/baz'},} == ra
    ...
    assert {'location': 'http://example.com/foo',
            'bar': {'id': 'http://example.com/#baz'},} == ra
...
def test_subscoped_id():
    ...
    assert {'id': 'http://example.com/#foo',
            'bar': {'id': 'http://example.com/#foo/bar/baz'},} == ra
...
def test_file_uri():
    ...
    assert "file:///foo/bar%20baz/quux" == schema_salad.ref_resolver.file_uri("/foo/bar baz/quux")
    assert os.path.normpath("/foo/bar baz/quux") == \
        schema_salad.ref_resolver.uri_file_path("file:///foo/bar%20baz/quux")
    assert "file:///foo/bar%20baz/quux%23zing%20zong" == schema_salad.ref_resolver.file_uri("/foo/bar baz/quux#zing zong")
    assert "file:///foo/bar%20baz/quux#zing%20zong" == schema_salad.ref_resolver.file_uri("/foo/bar baz/quux#zing zong", split_frag=True)
    assert os.path.normpath("/foo/bar baz/quux#zing zong") == \
