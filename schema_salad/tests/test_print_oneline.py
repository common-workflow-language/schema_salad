...
def test_print_oneline():
    ...
    with pytest.raises(ValidationException):
        try:
            ...
            assert msgs[0].endswith(src+":11:7: invalid field `invalid_field`, expected one of: 'loadContents', 'position', 'prefix', 'separate', 'itemSeparator', 'valueFrom', 'shellQuote'")
            assert msgs[1].endswith(src+":12:7: invalid field `another_invalid_field`, expected one of: 'loadContents', 'position', 'prefix', 'separate', 'itemSeparator', 'valueFrom', 'shellQuote'")
            print("\n", e)
            raise
...
def test_print_oneline_for_errors_in_the_same_line():
    ...
    with pytest.raises(ValidationException):
        try:
            ...
            assert msgs[1].endswith(src+":14:5: invalid field `aa`, expected one of: 'label', 'secondaryFiles', 'format', 'streamable', 'doc', 'id', 'outputBinding', 'type'")
            print("\n", e)
            raise
...
def test_for_invalid_yaml2():
    ...
    try:
        ...
        msg = reformat_yaml_exception_message(strip_dup_lineno(six.text_type(e)))
        assert msg.endswith(src+":2:1: expected <block end>, but found ':'") \
