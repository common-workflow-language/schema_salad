...
public class TypeDslLoader<T> implements Loader<T> {
    ...
    private Object resolve(final String doc_, final String baseUri, final LoadingOptions loadingOptions) {
        final Matcher m = TYPE_DSL_REGEX.matcher(doc_);
        if(m.matches()) {
            final String first = loadingOptions.expandUrl(
                m.group(1), baseUri, false, true, this.refScope
            );
            Object second = null;
            Object third = null;
