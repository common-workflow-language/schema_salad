...
RUN export SETUPTOOLS_SCM_PRETEND_VERSION=$(grep __version__ src/schema_salad/_version.py  | awk -F\' '{ print $2 }') ; source env3/bin/activate && SCHEMA_SALAD_USE_MYPYC=0 MYPYPATH=mypy-stubs python3 -m build --wheel --outdir=/wheels
